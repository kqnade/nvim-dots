...
'@
	...
	info -Msg "This script will install nvimdots to:"
	...
	if ($USE_SSH) {
		if ((check_nvim_version -RequiredVersionMin $REQUIRED_NVIM_VERSION)) {
			safe_execute -WithCmd { git clone --progress -b "$env:CCLONE_BRANCH" "$env:CCLONE_ATTR" 'git@github.com:kqnade/nvim-dots.git' "$env:CCDEST_DIR" }
			...
			safe_execute -WithCmd { git clone --progress -b 0.8 "$env:CCLONE_ATTR" 'git@github.com:kqnade/nvim-dots.git' "$env:CCDEST_DIR" }
		} else {
...
"@
		...
		if ((check_nvim_version -RequiredVersionMin $REQUIRED_NVIM_VERSION)) {
			safe_execute -WithCmd { git clone --progress -b "$env:CCLONE_BRANCH" "$env:CCLONE_ATTR" 'https://github.com/kqnade/nvim-dots.git' "$env:CCDEST_DIR" }
		} elseif ((check_nvim_version -RequiredVersionMin $REQUIRED_NVIM_VERSION_LEGACY)) {
			warn -Msg "You have outdated Nvim installed (< $REQUIRED_NVIM_VERSION)."
			info -Msg "Automatically redirecting you to the latest compatible version..."
			safe_execute -WithCmd { git clone --progress -b 0.8 "$env:CCLONE_ATTR" 'https://github.com/kqnade/nvim-dots.git' "$env:CCDEST_DIR" }
		} else {
			warn -Msg "You have outdated Nvim installed (< $REQUIRED_NVIM_VERSION_LEGACY)."
			_abort -Msg "This Neovim distribution is no longer supported." -Type "NotImplemented" -Info_msg @"
You have a legacy Neovim distribution installed.
...
- Project Homepage:
    https://github.com/kqnade/nvim-dots
    ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
